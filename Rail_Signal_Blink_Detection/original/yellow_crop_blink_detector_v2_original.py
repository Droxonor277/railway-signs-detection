"""
Samostatna diagnosticka analyza zluteho blikajiciho svetla z YOLO bbox cropu.

Pouziti:
    1) Uprav hodnoty v sekci CONFIG.
    2) Spust:
           python yellow_crop_blink_detector_v2.py

Skript nepotrebuje test_yellow_pipeline_bbox.py.
Vstupem je video + CSV z detection pipeline. Z CSV si vezme detection_ID,
z videa vyreze prislusne bbox cropy a z nich urci blikani zluteho svetla.

Navic uklada diagnostiku:
    - raw crop + normalizovany crop + yellow/white/active masky
    - CSV se vsemi signaly po framech
    - graf brightness/active/detrended/FFT
    - textovy report, proc algoritmus rozhodl dany stav
"""
...
# =============================================================================
# CONFIG - uprav tady, nic neni potreba predavat pres prikazovou radku
# =============================================================================
...
# CSV_PATH nech jako None = skript ho vytvori jako OUTPUT_DIR/<nazev_videa>.csv.
# Pokud chces pevnou cestu, nastav napr. r"C:/.../results/signal_4.csv".
CSV_PATH: Optional[str] = None

# Kdyz je True, skript nejdriv spusti detection pipeline a vytvori CSV,
# potom hned spusti analyzu zluteho blikani z tohoto CSV.
RUN_PIPELINE_BEFORE_ANALYSIS = True

# False = kdyz CSV uz existuje, pipeline se znovu nepousti.
# True = CSV se vytvori znovu pri kazdem spusteni.
FORCE_RECREATE_CSV = True

# Nastaveni detection pipeline.
DEVICE = "cuda:0"      # kdyz nemas GPU, nastav "cpu"
...
# Kdyz das None, vybere se nejdelsi track v CSV.
...
# Analyzuje se presny bbox z YOLO. Padding 0 = nepridavat okoli.
PADDING = 0.0
MIN_CROP_PX = 0
MAX_HOLD_FRAMES = 3

# Normalizace maleho cropu.
NORM_W = 64
NORM_H = 128

# Ocekavane frekvence zluteho blikani.
SLOW_HZ = 0.9
FAST_HZ = 1.8
MIN_HZ = 0.3
MAX_HZ = 5.0
...
# =============================================================================
# Detection pipeline - vytvoreni CSV pred analyzou
# =============================================================================

def resolve_csv_path() -> Path:
    """Vrati cestu k CSV. Pokud CSV_PATH=None, pouzije OUTPUT_DIR/<video_stem>.csv."""
    if CSV_PATH:
...
def load_pipeline_api():
    """Nacte DetectionPipeline/PipelineConfig z existujicich souboru projektu.

    Podporuje oba bezne zpusoby:
      1) projekt jako package: from pipeline import ...
      2) spousteni primo ze slozky s detection_pipeline.py a pipeline_config.py
    """
    try:
...
def pipeline_classify(crop: np.ndarray, det) -> tuple[str, float]:
    """Pipeline hook. CSV potrebujeme jen pro bboxy, proto vracime detekcni tridu."""
    return det.class_name, float(det.confidence)


def create_detection_csv_if_needed() -> Path:
    """Spusti detection pipeline a vytvori CSV, pokud je to povolene v CONFIG."""
    ...
    if csv_path.exists() and not FORCE_RECREATE_CSV:
        print(f"CSV uz existuje, pipeline preskakuji: {csv_path}")
        return csv_path
    ...
    if not video_path.exists():
        raise FileNotFoundError(f"Video neexistuje: {video_path}")
    if not model_path.exists():
        raise FileNotFoundError(f"Model neexistuje: {model_path}")

    output_dir.mkdir(parents=True, exist_ok=True)

    print("Spoustim detection pipeline pro vytvoreni CSV...")
    ...
    if not csv_path.exists():
        # Pipeline standardne uklada CSV podle stemu videa. Kdyz ma uzivatel CSV_PATH pevne jinam,
        # zkus najit standardni CSV a pripadne ho presunout/zkopirovat.
        ...
        if CSV_PATH and default_csv.exists() and default_csv != csv_path:
            ...
            raise FileNotFoundError(f"Pipeline dobehla, ale CSV nebylo nalezeno: {csv_path}")

    print(f"CSV pripraveno: {csv_path}")
    return csv_path
...
def select_track_id(detections_by_frame: dict[int, list[CsvDetection]], selected_id: Optional[str]) -> str:
    ...
    if not counts:
        raise RuntimeError("CSV neobsahuje zadne detekce pro zvoleny label.")
...
def collect_track_samples(
    ...
    if not track_frames:
        raise RuntimeError(f"Track detection_ID={track_id} nebyl v CSV nalezen.")
    ...
    if not cap.isOpened():
        raise OSError(f"Nelze otevrit video: {video_path}")
...
def get_video_frame_count(video_path: str | Path) -> int:
    """Vrati celkovy pocet snimku videa. Pokud to OpenCV nezjisti, vrati 0."""
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        return 0
...
# =============================================================================
# Extrakce zluteho/jasoveho signalu z cropu
# =============================================================================
...
def compute_masks(img_bgr: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, float, float, float]:
    ...
    # Zluta/oranzova; zamerne sirsi interval kvuli kamere a saturaci.
    ...
    # Presvicena/bilozluta cast lampy.
...
def extract_brightness_from_crop(crop_bgr: Optional[np.ndarray]) -> BrightnessSample:
    ...
    # Jas jen v aktivni masce. To je diagnosticky i prakticky dulezite:
    # pokud globalni jas neblika, ale aktivni oblast ano, chyba byla v mereni celeho bboxu.
    ...
    # Kombinovany signal: pri dostupne masce preferuje aktivni oblast, jinak fallback na globalni percentily.
...
def save_report(
    ...
    text.append("Interpretace vysledku:")
    if result.get("state") == "steady_yellow":
        text.append("- Algoritmus rozhodl steady_yellow, protoze nebyla potvrzena stabilni periodicka slozka blikani.")
        text.append("- Pokud jde o ocekavane blikajici video, zkontroluj track / bbox quality vyse.")
        text.append("- LOW_YOLO_DETECTION_COVERAGE, SHORT_TRACK, HIGH_BBOX_CENTER_JITTER nebo UNSTABLE_BBOX_SIZE znamena, ze problem je pravdepodobne v detekci/tracku, ne v prahu blikani.")
        text.append("- Pokud jsou track_quality_flags=OK a active mask trefuje lampu, pak je vhodne ladit rozhodovaci pravidla klasifikace.")
    elif result.get("state") == "uncertain":
        text.append("- Signal ma zmenu, ale frekvence neni stabilne v pasmu 0.9 Hz ani 1.8 Hz.")
        text.append("- U uncertain vysledku rozhoduj podle track_quality_score a podle grafu detrended/FFT.")
    else:
        text.append("- Blikani bylo potvrzeno. I tak zkontroluj track_quality_flags, pokud confidence vychazi nizko.")
...
def main() -> None:
    ...
    print(f"Analyzuji detection_ID={track_id}")
    ...
    if not samples:
        raise RuntimeError("Nebyly nacteny zadne cropy.")
    ...
    print(f"Pocet vzorku: {len(samples)}")
    print(f"Vysledek: {result['state']}")
    print(f"Frekvence: {result['frequency_hz']:.3f} Hz")
